<?php

namespace App\Providers\Filament;

use App\Filament\Pages\LaporanAnggaran;
use App\Filament\Pages\Pengmas\CetakLaporanKegiatanPerPilar;
use Filament\Http\Middleware\Authenticate;
use Filament\Http\Middleware\DisableBladeIconComponents;
use Filament\Http\Middleware\DispatchServingFilamentEvent;
use Filament\Pages;
use Filament\Panel;
use Filament\PanelProvider;
use Filament\Support\Colors\Color;
use Filament\Widgets;
use Illuminate\Cookie\Middleware\AddQueuedCookiesToResponse;
use Illuminate\Cookie\Middleware\EncryptCookies;
use Illuminate\Foundation\Http\Middleware\VerifyCsrfToken;
use Illuminate\Routing\Middleware\SubstituteBindings;
use Illuminate\Session\Middleware\AuthenticateSession;
use Illuminate\Session\Middleware\StartSession;
use Illuminate\View\Middleware\ShareErrorsFromSession;
use Filament\Navigation\NavigationGroup;
use App\Filament\Widgets\StatsOverview;
use App\Filament\Widgets\DataKegiatanChart;
use App\Filament\Widgets\InfoSistem;
use App\Filament\Pages\Auth\Loginan as CustomLogin; 


use App\Filament\Widgets\DataTesChart;

class AdminPanelProvider extends PanelProvider
{
    public function panel(Panel $panel): Panel
    {
        // dd('ADMIN PANEL PROVIDER DIJALANKAN!'); 
        return $panel
            ->default()
            
            ->brandName('CSR 1.1')
            // ->brandLogo(url('assets/pik2-logo.png'))
            ->brandLogoHeight('4rem')
            ->id('admin')
            ->path('admin')
            ->plugin(
                \MarcoGermani87\FilamentCaptcha\FilamentCaptcha::make()
            )
            ->login(CustomLogin::class)
            ->colors([
                'primary' => Color::Amber,
            ])

            ->discoverClusters(in: app_path('Filament/Clusters'), for: 'App\\Filament\\Clusters')
            ->discoverResources(in: app_path('Filament/Resources'), for: 'App\\Filament\\Resources')
            ->discoverPages(in: app_path('Filament/Pages'), for: 'App\\Filament\\Pages')
            ->navigationGroups([
                NavigationGroup::make()->label('Pemangku Kepentingan'),
                NavigationGroup::make()->label('Komunikasi, Publikasi, dan Hubungan Media'),
                NavigationGroup::make()->label('Pengembangan Masyarakat'),
                NavigationGroup::make()->label('Monitoring dan Evaluasi'),
                NavigationGroup::make()->label('Manajemen Pengguna'),
                NavigationGroup::make('Laporan') // Sesuaikan grup navigasi sesuai keinginan
                    ->label('Laporan'),
            ])
            ->pages([
                Pages\Dashboard::class,
                LaporanAnggaran::class,
                CetakLaporanKegiatanPerPilar::class,

            ])
            ->discoverWidgets(in: app_path('Filament/Widgets'), for: 'App\\Filament\\Widgets')
            ->widgets([
                    // Widgets\AccountWidget::class,
                    // Widgets\FilamentInfoWidget::class,

                    // InfoSistem::class,
                // StatsOverview::class,

                // DataTesChart::class,
                // DataKegiatanChart::class,

            ])
            ->middleware([
                EncryptCookies::class,
                AddQueuedCookiesToResponse::class,
                StartSession::class,
                AuthenticateSession::class,
                ShareErrorsFromSession::class,
                VerifyCsrfToken::class,
                SubstituteBindings::class,
                DisableBladeIconComponents::class,
                DispatchServingFilamentEvent::class,
            ])
            ->authMiddleware([
                Authenticate::class,
            ])
            ->sidebarCollapsibleOnDesktop()
            ;
    }

}
